...
def PLCC(pred, true):
    ...
    for i in range (len(pred)):
        a=pred[i].squeeze()
        b=true[i].squeeze()
        if len(np.unique(a)) == 1: print("pred is constant",a)
        if len(np.unique(b)) == 1: print("true is constant",b)
        tmp=pearsonr(a, b)[0]
...
class DNNModel(nn.Module):
    ...
    def forward(self, time_sequences, images, texts):
        ...
        combined_features = torch.cat((time_features,image_features), dim=1)
